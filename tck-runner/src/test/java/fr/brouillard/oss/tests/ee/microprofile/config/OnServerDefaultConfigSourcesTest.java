...
import fr.brouillard.oss.ee.microprofile.config.GuardEEConfigSources;
import fr.brouillard.oss.tests.ee.microprofile.config.sources.CopyOnBuildAggregationConfigSource;

public class OnServerDefaultConfigSourcesTest extends Arquillian {
    @Deployment
    public static WebArchive deploy() {
        JavaArchive implJar = ShrinkWrap
                .create(JavaArchive.class, "impl.jar")
                .addClass(CopyOnBuildAggregationConfigSource.class)
    ...
    public void can_retrieve_entries() throws IOException {
        CopyOnBuildAggregationConfigSource cs = new CopyOnBuildAggregationConfigSource(GuardEEConfigSources.defaultConfigSources());

        Assert.assertEquals(cs.getValue("my.int"), "1234");
        Assert.assertEquals(cs.getValue("my.string"), "hello");
    }
}
